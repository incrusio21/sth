...
frappe.ui.form.on("Sounding Stock CPO di BST", {
    ...
    ukuran_hasil_sounding(frm) {
        frm.events.get_ukuran_sounding(frm.doc.ukuran_hasil_sounding, "BST 01").then((res) => {
    ...
    ukuran_hasil_sounding_2(frm) {
        frm.events.get_ukuran_sounding(frm.doc.ukuran_hasil_sounding_2, "BST 02").then((res) => {
    ...
    get_ukuran_sounding(tinggi, bst) {
        const method = frappe.model.get_server_module_name(cur_frm.doctype) + ".get_ukuran_sounding"
        return frappe.xcall(method, { tinggi, bst })
    },

    get_berat_jenis_suhu(suhu) {
        frappe.call({
