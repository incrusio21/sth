...
def get_data(filters: Filters) -> list[dict]:
	...
	# Inject cuti bersama rows if leave_type is Cuti or no leave_type filter
	if not filters.get("leave_type") or filters.get("leave_type") == "Cuti":
		result = inject_cuti_bersama_rows(result, filters)
...
def inject_cuti_bersama_rows(result: list[dict], filters: Filters) -> list[dict]:
	Holiday = frappe.qb.DocType("Holiday")

	from_date, to_date = filters.get("from_date"), filters.get("to_date")

	employees_in_result = list(set([row.employee for row in result if row.leave_type == "Cuti"]))

	if filters.get("employee"):
		employees_in_result = [filters.get("employee")]

	if not employees_in_result:
		return result

	for employee in employees_in_result:
		employee_doc = frappe.db.get_value(
			"Employee", employee, ["employee_name", "holiday_list", "company"], as_dict=True
		)
		if not employee_doc or not employee_doc.holiday_list:
			continue

		# Get allocation period for this employee's Cuti
		...
		cb_dates = (
			...
			.where(
				(Holiday.parent == employee_doc.holiday_list)
				& (Holiday.cuti_bersama == 1)
		...
		cuti_bersama_count = len(cb_dates)

		# Inject cuti bersama rows
		for cb in cb_dates:
			result.append(frappe._dict({
				"leave_ledger_entry": None,
				"employee": employee,
				"employee_name": employee_doc.employee_name,
				"date": cb.holiday_date,
				"from_date": cb.holiday_date,
				"to_date": cb.holiday_date,
				"leave_type": "Cuti",
				"transaction_type": "Cuti Bersama",
				"transaction_name": cb.description or "Cuti Bersama",
				"leaves": -1,
				"is_carry_forward": 0,
				"is_expired": 0,
				"is_lwp": 0,
		...
		# Adjust expired leave rows for this employee
		if cuti_bersama_count > 0:
			for row in result:
				if (
					row.get("employee") == employee
					and row.get("leave_type") == "Cuti"
					and row.get("is_expired") == 1
					and row.get("transaction_type") == "Leave Allocation"
				):
					# Expired leaves are negative, reduce their magnitude by cuti bersama count
					# e.g. -10 expired + 3 cuti bersama = -7 expired
					adjusted = row["leaves"] + cuti_bersama_count
					# Cap at 0 — expired cannot become positive
					row["leaves"] = min(adjusted, 0)

	result = sorted(result, key=lambda x: (x.employee, x.leave_type, x.from_date or ""))
