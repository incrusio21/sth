...
def execute(filters=None):
	...
	query_l_promosi_dan_demosi = frappe.db.sql("""
		SELECT
		ep.employee_name as nama,
		"Promosi dan Demosi" as type,
		ep.promotion_date as tanggal,
		CASE
		...
		JOIN `tabEmployee Property History` as eph ON eph.parent = ep.name
		WHERE eph.property IN ("Company", "Unit", "Grade", "Employment Type") {};
  """.format(conditions), filters, as_dict=True)

	for emp_promosi_demosi in query_l_promosi_dan_demosi:
		data.append(emp_promosi_demosi)
