		...
		return frappe.call({
			...
			callback: function (r) {
				if (!r.exc) {
					...
					// Auto-fill due_date from syarat_pembayaran
					var doc = r.message;
					var po_name = null;
					if (doc && doc.items) {
						for (var i = 0; i < doc.items.length; i++) {
							if (doc.items[i].purchase_order) {
								po_name = doc.items[i].purchase_order;
								break;
							}
						}
					}

					if (po_name && cur_frm.doc.posting_date) {
						frappe.db.get_value('Purchase Order', po_name, 'syarat_pembayaran', function(data) {
							if (!data || !data.syarat_pembayaran) return;
							console.log(1)
							var match = data.syarat_pembayaran.match(/^(\d+)/);
							if (match) {

								
								var days = parseInt(match[1]);
								var due = frappe.datetime.add_days(cur_frm.doc.posting_date, days);
								cur_frm.set_value('due_date', due);
								console.log(due)
								frappe.show_alert({
									message: __('Due date otomatis: {0} hari → {1}', [days, due]),
									indicator: 'green'
								});
							}
						});
