...
class ReceivablePayableReport:
	...
	def get_invoice_details(self):
		...
		if self.account_type == "Receivable":
			...
			# ambil Sales Order dari Sales Invoice Item
			si_names = [d.name for d in si_list]
			if si_names:
				so_links = frappe.db.sql("""
					SELECT DISTINCT
						sii.parent,
						sii.sales_order,
						so.no_kontrak_external
					FROM `tabSales Invoice Item` sii
					LEFT JOIN `tabSales Order` so
						ON so.name = sii.sales_order
					WHERE sii.docstatus = 1
						AND sii.parent IN %(si_names)s
						AND sii.sales_order IS NOT NULL
						AND sii.sales_order != ''
				""", {"si_names": si_names}, as_dict=1)

				for d in so_links:
					if d.sales_order and d.parent in self.invoice_details:
						self.invoice_details[d.parent].setdefault(
							"no_kontrak_penjualan",
							d.sales_order
						)

						self.invoice_details[d.parent].setdefault(
							"no_kontrak_external",
							d.no_kontrak_external
						)
	...
	def set_party_details(self, row):
		if not row.party:
			return
		# simpan docname asli sebelum ditimpa, kalau-kalau dibutuhkan di tempat lain
		row.party_name_link = row.party

		party_details = self.get_party_details(row.party) or {}
		row.update(party_details)

		if self.account_type == "Receivable":
			display_name = party_details.get("customer_name")
		else:
			display_name = party_details.get("supplier_name")

		# override kolom party: pakai nama custom kalau ada, fallback docname
		if display_name:
			row.party = display_name
		# else: row.party tetap docname (tidak diubah)
	...
	def get_columns(self):
		...
		if self.account_type == "Receivable":
			...
			self.add_column(
				label=_("No Kontrak Penjualan"),
				fieldname="no_kontrak_penjualan",
				fieldtype="Link",
				options="Sales Order",
				width=180,
			)
