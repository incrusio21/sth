...
def format_report_data(filters: Filters, item_details: dict, to_date: str) -> list[dict]:
	...
	for _item, item_dict in item_details.items():
		...
		# Tanggal acuan: kalau ada transaksi keluar terakhir, pakai itu.
		# Kalau belum pernah keluar, fallback ke tanggal slot FIFO asli (perilaku lama).
		last_out_date = item_dict.get("last_out_date")

		if last_out_date:
			average_age = date_diff(to_date, last_out_date)
			earliest_age = date_diff(to_date, last_out_date)
			latest_age = date_diff(to_date, last_out_date)
		else:
			average_age = get_average_age(fifo_queue, to_date)
			earliest_age = date_diff(to_date, fifo_queue[0][1])
			latest_age = date_diff(to_date, fifo_queue[-1][1])

		range_values = get_range_age(filters, fifo_queue, to_date, item_dict, last_out_date)
		...
		row = [
			...
			details.item_name,
			details.description,
			details.kode_kelompok_barang,
			details.kelompok_barang,
			details.kode_sub_kelompok_barang,
			details.sub_kelompok_barang,
			details.brand,
		]
...
def get_range_age(
	filters: Filters, fifo_queue: list, to_date: str, item_dict: dict, last_out_date=None
) -> list:
	precision = cint(frappe.db.get_single_value("System Settings", "float_precision", cache=True))
	range_values = [0.0] * ((len(filters.ranges) * 2) + 2)

	for item in fifo_queue:
		# Kalau ada last_out_date, semua slot yang masih tersisa
		# dianggap berumur sejak transaksi keluar terakhir.
		# Kalau belum pernah keluar, pakai tanggal slot FIFO asli (perilaku lama).
		reference_date = last_out_date or item[1]

		age = flt(date_diff(to_date, reference_date))
...
def get_chart_data(data: list, filters: Filters) -> dict:
	...
	data.sort(key=lambda row: flt(row[8]), reverse=True)
...
class FIFOSlots:
	...
	def __update_balances(self, row: dict, key: tuple | str):
		...
		# Catat tanggal transaksi KELUAR (actual_qty < 0) yang paling baru.
		# Karena SLE diproses berurutan (orderby posting_datetime, creation),
		# nilai ini akan otomatis menjadi tanggal keluar TERAKHIR.
		if row.actual_qty < 0:
			self.item_details[key]["last_out_date"] = row.posting_date
	...
	def __aggregate_details_by_item(self, wh_wise_data: dict) -> dict:
		...
		for key, row in wh_wise_data.items():
			...
			# ambil tanggal keluar paling baru antar warehouse
			last_out = row.get("last_out_date")
			if last_out and (
				not item_row["last_out_date"] or last_out > item_row["last_out_date"]
			):
				item_row["last_out_date"] = last_out
	...
	def __get_stock_ledger_entries(self) -> Iterator[dict]:
		...
		sle_query = (
			...
			.select(
				...
				item.kelompok_barang,
				item.kode_sub_kelompok_barang,
				item.sub_kelompok_barang,
				item.brand,
				item.description,
				item.stock_uom,
				item.has_serial_no,
	...
	def __get_item_query(self):
		...
		if self.filters.get("item_code"):
				query = query.where(item.name == self.filters.get("item_code"))
