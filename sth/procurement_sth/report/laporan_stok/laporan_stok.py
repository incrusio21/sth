...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		if sle.get("date"):
			sle.date = str(sle.date.year)

		sle.tanggal_terakhir_po = get_latest_date_po(sle.item_code)
		sle.jumlah_hari = get_jumlah_hari(sle.item_code, sle.warehouse, filters.get("to_date"))
...
def get_latest_date_po(item_code):
  ...
  return query[0].transaction_date if query else {}

def get_jumlah_hari(item_code, warehouse, to_date):
	"""
	Menghitung jumlah hari sejak transaksi stock terakhir
	pada item dan warehouse tertentu.
	"""

	last_posting_date = frappe.db.get_value(
		"Stock Ledger Entry",
		filters={
			"item_code": item_code,
			"warehouse": warehouse,
	...
	return (getdate(to_date) - getdate(last_posting_date)).days
...
def get_columns(filters):
	columns = [
		{"label": _("Periode"), "fieldname": "date", "fieldtype": "Data", "width": 100},
		{
			"label": _("Warehouse"),
			"fieldname": "warehouse",
			"fieldtype": "Link",
		...
		{"label": _("Nama Barang"), "fieldname": "item_name", "width": 100},
		{
